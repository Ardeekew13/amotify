...
export const applyRoundingDifference = (
	...
	const lastAutoSplitIndex = members.findIndex(
		(m, index) =>
			...
			index ===
				members.length -
					...
					[...members]
						.reverse()
						.findIndex((rm) => autoSplitMemberIds.includes(rm._id)),
	);
...
export const formatToTwoDecimals = (num: number): string => num?.toFixed(2);
...
export const distributePercentages = (
	...
	if (difference > 0) {
		...
		for (let i = 0; i < centsToDistribute && i < fractionalParts.length; i++) {
			...
			roundedPercentages[memberIndex] = roundToTwoDecimals(
				roundedPercentages[memberIndex] + 0.01,
			);
		}
	}
